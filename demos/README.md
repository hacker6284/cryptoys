...
Scramble can also hash a **file** (`shared/file-hash.js`, `scramble/hash-worker.js`). File is a separate path: first ceiling **10 MB**, never dumped into Message. A paperclip sits at the end of the Message line; filename + size is a **full-width row under Message**. Pick auto-starts a raw-byte hash (PNG/JPEG included). The worker (`createSilentHasher`) is given ~1s to come ready; if it does not, host silent hash still writes Digest. A quiet progress bar tracks chunk progress and clears when Digest lands. Play / teach stay off above 4 KiB. `setAlg` still waits for Play. Oversized picks are rejected with a quiet note. Clear/replace returns to typed Message. DoubleDeal Message is cipher plaintext, not a hash input — no file path there.
