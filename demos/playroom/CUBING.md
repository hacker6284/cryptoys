...
(`shared/live-digest.js`). A File pick hashes raw bytes on a fast JS
cube (`createFastHasher`) and writes Digest when that walk finishes;
`setAlg` still waits for Play on modest files (≤ 4 KiB) and
