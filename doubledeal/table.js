...
export const PASS_Z = 1.5 * ROW_PITCH + CARD_D + 0.6;
...
export function createCardTable({ parent, faces, navy, red } = {}) {
    ...
    function teachPoints(kind) {
        ...
        return gridPoints(MESSAGE_X);
    }

    function tablePoints() {
        return gridPoints(MESSAGE_X).concat(gridPoints(KEY_X));
    }

    function dispose({ textures = false } = {}) {
        generation += 1;
        const owned = [...message, ...key, marker, ...hilites];
        ...
        if (textures) {
            for (const tex of faces) tex?.dispose();
            navy?.dispose();
            red?.dispose();
        }
    }
