...
assert.equal(clock.dead(gen), false);
let stepped = 0;
await clock.tween(400, (t) => {
    stepped = t;
}, { generation: gen });
assert.equal(stepped, 1, "reduced-motion snaps the beat");
clock.skip();
assert.equal(clock.dead(gen), true, "skip invalidates the live generation");

console.log("beat-clock tests ok");
