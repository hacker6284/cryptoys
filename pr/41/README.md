...
Motion proof strips (agents / review): `?debugCapture=1` samples the canvas on named beats and every 240 ms. It is a no-op without that flag. Headless:
...
Sheets land in `$CAPTURE_OUT` (default `/opt/cursor/artifacts/strips`) as `{tag}_{algo}-{enter|leave}_strip.png`. Do not claim enter/leave smoothness without a strip.
