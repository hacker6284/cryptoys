...
Scramble can also hash a **file** (`shared/file-hash.js`, `scramble/hash-worker.js`). File is a separate path: first ceiling **10 MB**, never dumped into Message. A paperclip sits at the end of the Message line; filename + size appear on the next row. Pick auto-starts a raw-byte hash (PNG/JPEG included) in the worker (`createSilentHasher` — same turns as `update`/`evaluate`, no teach steps) and writes Digest hex when done. Play / teach stay off above 4 KiB so the leave list cannot freeze the room. `setAlg` still waits for Play. Oversized picks are rejected with a quiet note. Clear/replace returns to typed Message. DoubleDeal Message is cipher plaintext, not a hash input — no file path there.
