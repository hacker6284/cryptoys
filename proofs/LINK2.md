...
| MegaDreifach arbitrary-width limb algebra, one `big_mul` digit, one `big_divmod_small` step | **PARTIAL** (`natLimbs`, `scanMul`, `divLE`, `jDigitAt`, `divGenStep_at`, `big_mul_left_loop`, `big_mul_zero_right`, `big_divmod_empty`). Canonical base-`10^9` limbs are defined for every natural number. Schoolbook multiply-by-one-limb and high-first division agree with `limbVal`. One emitted multiply digit (left factor one limb, cell still zero) and one emitted division step (any index, divisor `< 10^9`) match that algebra, and the one-limb-left `big_mul` reduces to its schoolbook loop. The induction that stacks those steps into `big_mul`, `big_divmod_small`, `big_factorial` through `51!`, 28-byte `big_from_be`, `peel_leading`, or `phi_chunk` is not closed. Not `51!`. Not `phi_chunk`. Not `phi_inv`. Not `v_Hash`. |
| MegaDreifach full `v_Hash` / Scramble algebraic ≃ Generated | **OPEN** (`v_Hash` is M13; Scramble has little ledger). The pad, compose, `require_permutation`, `pack_ori2`, `pack_ori3`, length-20 `even_perm_rank_big`, zero-rank `position_to_bytes`, zero-byte `big_from_be`, short `big_from_be` (`BeShortWf`, length `≤ 3`), two-limb `big_from_be` (`BeLimb2Wf`, length `≤ 7`), `big_factorial` (`n ≤ 26`), and `peel_leading` of every rank below `limbCap d` for `d ≤ 26` (every digit `q`, three limbs once `d ≥ 20`) are closed. The arbitrary-width limb model (`natLimbs`, `scanMul`, `divLE`) and one emitted multiply digit / division step are in place; the loop induction that would reach `51!`, a 28-byte `big_from_be`, `peel_leading` for `d ≤ 51`, and `phi_chunk` / `phi_inv` is not closed. Positive `range_list` (`0 < n`, `FitsLen`, including 52) is already `range_list_refines` in `EvenRank.lean`. A positive corner or edge rank is outside the proved limb fragment. Full `v_Hash` remains OPEN. |
