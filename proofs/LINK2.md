...
| MegaDreifach full `v_Hash` / Scramble algebraic ≃ Generated | **OPEN** (`v_Hash` is M13; Scramble has little ledger). The pad, compose, `require_permutation`, `pack_ori2`, `pack_ori3`, length-20 `even_perm_rank_big`, and zero-rank `position_to_bytes` slices are closed. `phi_chunk` / `phi_inv` are still open. A positive corner or edge rank is outside the proved limb fragment. |
