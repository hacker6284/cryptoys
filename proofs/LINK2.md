...
| MegaDreifach full `v_Hash` / Scramble algebraic ≃ Generated | **OPEN** (`v_Hash` is M13; Scramble has little ledger). The pad, compose, `require_permutation`, `pack_ori2`, `pack_ori3`, and length-20 `even_perm_rank_big` slices are closed. `phi_chunk` / `phi_inv` and `position_to_bytes` are still open. |
