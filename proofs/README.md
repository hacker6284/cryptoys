...
4. **Link 2 refinement** — algebraic skeleton equals the generated program on the well-formed domain. DoubleDeal: `drop_front` / `push_front` / `left_rotate`; `Generated.passkey` ≃ `passToKeyCutFallback` on every well-formed list. NEXT is `passkey_inv` (same twin / `runLoopOn` pattern). Not emitter soundness (that stays layer-2 TAP / Link 1). See [`LINK2.md`](LINK2.md).
