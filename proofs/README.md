...
gate is on at emit for DoubleDeal, MegaDreifach, and Scramble. All three
publics (DD / MD / Scramble) are terminates-ready and have Generated Lean.
...
2. **Generated TAP** — `sudoc emit-ir --require terminates` → protocol-4 Lean backend → `lake` → TAP, from the same `.sudo` that JS/Python compile. DoubleDeal 10/10 (two test-only kind-scan `while`s stripped under the gate; JS still runs all twelve), MegaDreifach 11/11, Scramble 15/15. Evidence the emitter ran, **not** a sudo=Lean theorem.
