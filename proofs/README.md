...
4. **Link 2 refinement** — algebraic skeleton equals the generated program on the well-formed domain. DoubleDeal: `drop_front` / `push_front` / `left_rotate`; `passkey` on length ≤ 1; one PassKey step ≃ `passKeyStep` and twin-loop induction on every well-formed list. Residual `Generated.passkey` glue on all lists is OPEN. Not emitter soundness (that stays layer-2 TAP / Link 1). See [`LINK2.md`](LINK2.md).
...
| --- | --- | --- |
| DoubleDeal | `primitives/cipher/doubledeal/` | **Generated** Lean under `doubledeal/lean/Generated/` (from `doubledeal.sudo`; TAP 10/10 under the terminates gate). Proof-only stones under `doubledeal/lean/DoubleDeal/`. PassKey injectivity is **proved** on the list model (PR #3). Link 2: `Generated.passkey` ≃ that model on length ≤ 1; `left_rotate` ≃ `rotL`; one step ≃ `passKeyStep` and twin-loop induction on every well-formed list. Residual `Generated.passkey` glue and `Generated.encrypt` remain OPEN. |
