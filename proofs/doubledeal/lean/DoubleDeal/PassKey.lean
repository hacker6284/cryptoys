/-
  PROOF-ONLY (SPEC §6 S3 / S4). Sudo does not emit these theorems.

  The algorithm is Generated.passkey / Generated.passkey_inv (from
  doubledeal.sudo). This file is a Fin/List model so the inverse lemmas
  typecheck. Link 2 (`DoubleDeal/Link2/PassKey.lean`) glues
  `Generated.passkey` ≃ `passToKeyCutFallback` on every well-formed list.
  NEXT is `passkey_inv` (same twin / `runLoopOn` pattern). Do not edit
  this file to "fix encrypt".
-/
