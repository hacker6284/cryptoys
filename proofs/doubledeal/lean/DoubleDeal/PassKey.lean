/-
  ...
  typecheck. Link 2 starts the refinement (`DoubleDeal/Link2/PassKey.lean`):
  one generated step ≃ `passKeyStep` and the twin-loop induction are
  proved; `Doubledeal.passkey` residual glue on every list is still OPEN.
  Do not edit this file to "fix encrypt".
-/
