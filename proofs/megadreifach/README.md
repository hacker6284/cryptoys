...
| — | Full `Generated.v_Hash` refinement; Scramble algebraic ≃ Generated | OPEN. Pad, `compose`, `require_permutation`, `pack_ori2`, `pack_ori3`, length-20 `even_perm_rank_big` (`Rank20Wf`), zero-rank `position_to_bytes` (`PosBytesWf`), zero-byte `big_from_be`, short `big_from_be` (`BeShortWf`, length `≤ 3`), `big_factorial` (`n ≤ 19`), zero `peel_leading` (`d ≤ 19`), and one-limb `peel_leading` (`d ≤ 12`, `n < 10^9`) are closed. `phi_chunk` / `phi_inv` are still open. Not collision resistance. |
