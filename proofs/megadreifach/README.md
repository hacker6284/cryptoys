...
| Link 2 | Arbitrary-width base-`10^9` limbs; one `big_mul` digit and one `big_divmod_small` step | Proved as algebra (`natLimbs`, `scanMul`, `divLE`) plus `jDigitAt`, `divGenStep_at`, `big_mul_left_loop`. Not the stacked `big_mul` / `big_factorial` / 28-byte `big_from_be`. Not `phi_chunk`. Not `v_Hash`. |
