...
| Link 2 | `big_mul` of a wide left factor by one limb, outer loop | Exposed (`big_mul_wide_open`, `mulAcc`, `kLoop_write`). The digit induction into `natLimbs (q * n)` is open. Not `51!`. Not `v_Hash`. |
...
| — | Full `Generated.v_Hash` refinement; Scramble algebraic ≃ Generated | OPEN. Pad, `compose`, `require_permutation`, `pack_ori2`, `pack_ori3`, length-20 `even_perm_rank_big` (`Rank20Wf`), zero-rank `position_to_bytes` (`PosBytesWf`), zero-byte `big_from_be`, short and two-limb `big_from_be`, `big_factorial` (`n ≤ 26`), `peel_leading` below `limbCap d` for `d ≤ 26`, and arbitrary-width `big_divmod_small` (one-limb divisor) are closed. Wide-by-small `big_mul` is reduced to its schoolbook loop; `natLimbs (q * n)`, `51!`, and `phi_chunk` / `phi_inv` are still open. Full `v_Hash` is still open. Not collision resistance. |
