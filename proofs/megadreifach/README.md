...
Normative product name: **MegaDreifach**. The puzzle/group library stays **megaminx**. Soft-lock parameters (pad B=28, φ, abs-G2 + F3 t=12, IV-COOK12, 29-byte digest, 3-solve DM) live in the research notes that seeded this package; the Lean model is the obligation ledger, not a second specification.
...
| **(b) Vector agreement** | KAT *metadata* (pad lengths, block counts, digest width, `\|G\|`) in `vectors/` checked by `lake exe megadreifach`. Full digest equality needs the abs-G2 runner. | Compiled Lean evaluation. **Not** kernel `decide`. **Not** a proof that Python = Lean. |
| **(c) Future: emitter / full Hash** | A Lean `E_m` (abs-G2 + F3) plus a sudocode emitter. | Does not exist yet. Do not read (b) as a substitute. |
...
| M8 (nets) | Pairwise distinctness of the 60×52 concrete face-turn nets | Open; computer-checked in research `g2_proof_core.py`, too large for kernel `decide` |
| M9 | Abs-G2 L2 mid-block: no 2-card local collision | OPEN (plan in STONES.md). Informal proof in research `G2_PROOF.md` |
| M13 | Lean digests of exported KATs equal `kats/megaminx_hash_kats.json` | OPEN; metadata only in this drop. Needs `E_m` |
...
M1 → M2 → M4 → M5 → M7 → M3 (packing) → M6 → M10/M11/M12 → M8 reduction. Leave M8 nets, M9, and M13 for follow-up PRs. Do not promote pressure tables or L3-absence slogans.
