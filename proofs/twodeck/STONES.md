...
| S4 | PassKey injectivity on lists / \(S_{52}\) | **Proof** | `PassKey.lean`: `invPassKeyStep_passKeyStep`, `passKeyStep_invPassKeyStep`, `passKey_leftInverse`, `passKey_rightInverse`, `passKey_injective` | Constructive inverse. Cycle structure / orbit lengths stay **evidence only**. SPEC §3.7 still says open; update that sentence in a follow-up. |
