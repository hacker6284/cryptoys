...
| S2 | Round-trip: `inv_full_round ∘ full_round`, `inv_final ∘ final`, `decrypt ∘ encrypt` | **Proof** (abstract keys + concrete schedule) | `lean/TwoDeck/Round.lean`: `invUnkeyedWithMix_rt`, `encrypt1WithMix_rt`, `encryptNoMix_rt`, `invFullRound_fullRound`, `invFullRoundNoMix_fullRoundNoMix`, `encryptN_rt`, `encrypt6_rt`. `Concrete.lean`: `encryptDeckFn_rt`, `encryptDeckFn_rt_range`, `encryptDeck_eq_encryptDeckFn`, `decryptDeck_eq_decryptDeckFn` | Round keys may be hypothesized as `pos` / `invPos` bijections. The real PassKey schedule (`passKeyIter`, `keyPos`) is a `Perm52` bijection, so `encrypt6_rt` applies. List KATs run `encryptDeck` / `decryptDeck`, which equal the proved packet maps. End-to-end KATs vs `twodeck.sudo` are **evidence** (`lake exe twodeck`), not a sudo=Lean proof. |
...
| S5 | CTR factoradic unranking bijection \(\mathbb{Z}/13!\mathbb{Z} \leftrightarrow S_{13}\) | **Partial proof** | `lean/TwoDeck/Factoradic.lean`: `unrankPerm_perm`, `unrankPerm_inj_3` | Permutation of items is proved. Injectivity is kernel `decide` on `Fin 3!` only. |
