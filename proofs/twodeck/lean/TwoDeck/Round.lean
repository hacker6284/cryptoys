/-
  Round / encrypt structure for TwoDeck (Nr = 6).
...
def encryptNoMix (m : Fin 52 → Nat)
    ...
    Fin 52 → Nat :=
  let m1 := TwoDeck.composeVec 52 Nat m pos0
...
def decryptNoMix (c : Fin 52 → Nat)
  ...
  TwoDeck.composeVec 52 Nat m1 invPos0
...
theorem encryptNoMix_rt (m : Fin 52 → Nat)
  ...
  have step1 :
      (fun i =>
        TwoDeck.composeVec 52 Nat (unkeyedNoMix (TwoDeck.composeVec 52 Nat m pos0)) pos1
          (invPos1 i)) =
      unkeyedNoMix (TwoDeck.composeVec 52 Nat m pos0) := by
    funext i
    simp only [TwoDeck.composeVec, h1R]
  rw [step1, invUnkeyedNoMix_unkeyedNoMix]
  funext j
  simp only [TwoDeck.composeVec, h0R]
...
theorem encrypt1WithMix_rt (m : Fin 52 → Nat)
  ...
  have step1 :
      (fun i =>
        TwoDeck.composeVec 52 Nat (unkeyedWithMix (TwoDeck.composeVec 52 Nat m pos0)) pos1
          (invPos1 i)) =
      unkeyedWithMix (TwoDeck.composeVec 52 Nat m pos0) := by
    funext i
    simp only [TwoDeck.composeVec, h1R]
...
theorem invFullRound_fullRound (m : Fin 52 → Nat)
  ...
  have step :
      (fun i => TwoDeck.composeVec 52 Nat (unkeyedWithMix m) pos (invPos i)) =
      unkeyedWithMix m := by
    funext i
    simp only [TwoDeck.composeVec, hR]
...
theorem invFullRoundNoMix_fullRoundNoMix (m : Fin 52 → Nat)
  ...
  have step :
      (fun i => TwoDeck.composeVec 52 Nat (unkeyedNoMix m) pos (invPos i)) =
      unkeyedNoMix m := by
    funext i
    simp only [TwoDeck.composeVec, hR]
...
/-- TwoDeck encrypt with `nMix` MixColumns rounds then one final no-mix round.
...
def encryptN (nMix : Nat) (m : Fin 52 → Nat)
  ...
  let m0 := TwoDeck.composeVec 52 Nat m pos0
...
def decryptN (nMix : Nat) (c : Fin 52 → Nat)
  ...
  TwoDeck.composeVec 52 Nat m0 invPos0
...
theorem encryptN_rt (nMix : Nat) (m : Fin 52 → Nat)
  ...
  have peelFinal :
      invFullRoundNoMix
        (fullRoundNoMix (applyFullRounds nMix (TwoDeck.composeVec 52 Nat m pos0) posMix)
          posFinal)
        posFinal invPosFinal =
      applyFullRounds nMix (TwoDeck.composeVec 52 Nat m pos0) posMix :=
  ...
  have peelMix :
      applyInvFullRounds nMix
        (applyFullRounds nMix (TwoDeck.composeVec 52 Nat m pos0) posMix)
        posMix invPosMix =
      TwoDeck.composeVec 52 Nat m pos0 :=
    applyInvFullRounds_applyFullRounds nMix (TwoDeck.composeVec 52 Nat m pos0)
